...
# === Setup ===
data_dir = "./scripts/result/test_frequency_paras"
output_dir = os.path.join(data_dir, "fig")
os.makedirs(output_dir, exist_ok=True)

grouped_files = defaultdict(list)
pattern = re.compile(r"^(periodic.*)_frequency_hc20_.*\.csv$")

for filename in os.listdir(data_dir):
    if filename.endswith(".csv") and filename.startswith("periodic"):
...
for prefix, files in grouped_files.items():
    ...
    for idx, filename in enumerate(sorted(files)):
        ...
        if {'LocalCount', 'RemoteCount', 'PmemCount'}.issubset(df.columns):
            ...
            config = filename.split("_frequency_")[1].replace(".csv", "")
            color = colors(idx)
            axes1[0].plot(time, df["LocalCount"], label=config, color=color)
            axes1[1].plot(time, df["RemoteCount"], label=config, color=color)
            axes1[2].plot(time, df["PmemCount"], label=config, color=color)
    ...
    for idx, filename in enumerate(sorted(files)):
        ...
        if {'LocalAccess', 'RemoteAccess', 'PmemAccess'}.issubset(df.columns):
            ...
            config = filename.split("_frequency_")[1].replace(".csv", "")
            color = colors(idx)
            axes2[0].plot(time, df["LocalAccess"], label=config, color=color)
            axes2[1].plot(time, df["RemoteAccess"], label=config, color=color)
            axes2[2].plot(time, df["PmemAccess"], label=config, color=color)
    ...
    for idx, filename in enumerate(sorted(files)):
        ...
        if "Latency(ns)" in df.columns:
            ...
            config = filename.split("_frequency_")[1].replace(".csv", "")
            color = colors(idx)
            ax3.plot(time, df["Latency(ns)"], label=config, color=color)
    ...
    for idx, filename in enumerate(sorted(files)):
        ...
        if "Throughput(ops/s)" in df.columns:
            ...
            config = filename.split("_frequency_")[1].replace(".csv", "")
            color = colors(idx)
            ax4.plot(time, df["Throughput(ops/s)"], label=config, color=color)
